...
export function Order({
  ...
  const ordersOfIngredients = useMemo(
    () =>
      order.ingredients
        .map((id) => ingredients.find((item: IIngredient) => item._id === id))
        ...
        <div className={orderStyles.ingredientsContainer}>
          <ul className={orderStyles.ingredients}>
            {uniqueIngredients.map((item, index) => {
              if (index < 5)
                return (
                  <li key={uuid()} className={orderStyles.ingredientEl}>
                    <img key={uuid()} src={item.image} alt="" width={110} />
                  </li>
                );
              if (index === 5)
                return (
                  <Overlay
                    key={uuid()}
                    countOfIngredients={ordersOfIngredients.length}
                  >
                    <li key={uuid()} className={orderStyles.ingredientEl}>
                      <img key={uuid()} src={item.image} alt="" width={110} />
                    </li>
